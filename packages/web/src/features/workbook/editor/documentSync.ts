...
export function buildDocumentOperations(
  ...
  for (const address of addresses) {
    ...
    if (renderRow < headerRows) continue;
    const row = renderRow - headerRows;
  ...
  const previousMerges = extractMergesFromCelldata(previous)
    .filter((merge) => merge.row[0] >= headerRows)
    .map((merge) => ({
      startRow: merge.row[0] - headerRows,
      startCol: merge.col[0],
      endRow: merge.row[1] - headerRows,
      endCol: merge.col[1],
    }));
