...
export type ChartOverlayInteraction = {
  chartId: string;
...
export function chartRectWithMinimumSize(
  chart: ChartSpec,
  layout: SheetGridLayout,
  interaction: ChartOverlayInteraction | null,
): ChartRect {
  const rect =
...
export function useChartOverlayInteraction({
  activeSheetId,
  charts,
  layout,
  workspaceId,
  ...
  const interactionRef = useRef<ChartOverlayInteraction | null>(null);
  ...
  const updateInteraction = useCallback(
    (next: ChartOverlayInteraction | null, immediate = false) => {
  ...
  useEffect(() => {
    ...
    setAnchorOverrides({});
    setError(null);
  }, [activeSheetId, updateInteraction]);
  ...
  }, [removeChart, selectedId, updateInteraction]);
  ...
  const beginInteraction = useCallback(
    (event: React.PointerEvent, chartId: string, mode: InteractionMode, rect: ChartRect) => {
      event.preventDefault();
      event.stopPropagation();
      setError(null);
