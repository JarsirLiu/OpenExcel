    ...
    if (delta && typeof delta === "object") {
      ...
      if (d.type === "write" && Array.isArray(d.cells)) {
        ...
        for (const op of d.operations) {
          if (!op || typeof op !== "object") continue;
          const o = op as Record<string, unknown>;
          if (typeof o.startRow === "number" && typeof o.startCol === "number" && typeof o.endRow === "number" && typeof o.endCol === "number") {
            for (let r = o.startRow; r <= o.endRow; r++) {
              for (let c = o.startCol; c <= o.endCol; c++) {
                entry.changedCells.add(`${r},${c}`);
                entry.changeCount++;
...
export function SheetChangeSummary({ parts, onNavigateSheet }: { parts: any[]; onNavigateSheet?: (sheetId: number) => void }) {
  ...
  return (
    <div style={{ marginTop: 12 }}>
      <div style={{ fontSize: 12, fontWeight: 600, color: "var(--muted-foreground)", marginBottom: 6 }}>
        ...
        <div key={sheet.sheetId} style={{ marginBottom: 4 }}>
          <div
            style={{
              ...
              padding: "6px 8px", background: "var(--muted)",
          ...
          >
            <span style={{ fontWeight: 500, color: "var(--foreground)" }}>
              {sheet.sheetName}{sheet.sheetNo != null ? ` (#${sheet.sheetNo})` : ""}
            </span>
            <span style={{ color: "var(--hint-foreground)", fontSize: 12 }}>
              {sheet.changeCount} 处改动
            </span>
            <span style={{ marginLeft: "auto", flexShrink: 0, color: "var(--muted-foreground)", fontSize: 10, cursor: "pointer", padding: "2px 4px", borderRadius: "var(--radius-sm)", transition: "transform 0.15s", transform: expandedSheets.has(sheet.sheetId) ? "rotate(90deg)" : "rotate(0deg)" }}
              onClick={(e) => { e.stopPropagation(); toggleSheet(sheet.sheetId); }}
            >
              ▶
            </span>
          </div>
