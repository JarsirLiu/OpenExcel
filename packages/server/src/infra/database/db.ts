...
import { getDatabaseTransactionOptions } from "./databaseConcurrency.js";
...
async function createPrismaClient(config = loadDatabaseConfig()): Promise<PrismaClientLike> {
  ...
  const transactionOptions = getDatabaseTransactionOptions();

  switch (provider) {
    case "sqlite": {
      const { PrismaClient } = await import("../../../prisma/generated/sqlite/client/index.js");
      return new PrismaClient({ datasources: datasource, transactionOptions });
    }
    case "postgresql": {
