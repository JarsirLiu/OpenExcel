...
日期是单元格值的语义，不是模型需要自行计算的数字格式。权威 Sheet 快照继续保存 Excel
序列号和原始格式：
...
`readSheetData` 保持 `values` 的紧凑二维结构，因此日期在其中仍是 `44805`；同时只为当前
读取范围内的日期单元格返回稀疏的 `dateValues`：

```json
{
  "values": [[44805, 100]],
  "dateValues": { "A1": "2022-09-01" }
}
```

`dateValues` 是模型读取投影，不是第二份持久化事实；没有日期时省略该字段。日期时间使用
无时区的 `YYYY-MM-DD` 或 `YYYY-MM-DD HH:mm:ss`，不使用 JavaScript `Date` 或带 `Z` 的
UTC 字符串，避免时区偏移。`findSheetCells` 可以使用 `valueType: "date"` 定位日期单元格。

模型通过 `writeCells` 修改日期时必须显式传 `valueType: "date"`，例如：
...
Core 负责把日期文本转换为 Excel 序列号、设置 `ct.t = "d"`，并沿用已有日期格式；模型
不得直接写入序列号来猜测日期。公式仍按公式契约处理，公式结果是否为日期由其缓存值和
日期格式共同决定。导出始终使用快照中的 `v + ct`，不使用 `dateValues` 回写。
