...
7. **可扩展**：后续可以增加格式、条件格式、图表、透视表、批注和命名区域，而不修改基础数据契约。
...
- 用二维 `values` 替换旧的稀疏 `data` 返回结构；
- 在同一数据工具结果中加入压缩公式模式；
- 正确表达空值、数字零和合并区域；
- 实现确定性的结构化 `continuation` 分页；
- 删除旧返回结构和相关兼容分支。

### 第二阶段

- 新增 `findSheetCells` 的值、公式和直接格式查询；
- 统一 RGB/ARGB、索引色和主题色的 core 表示；
- 增加条件格式查询与 Excel 对象摘要的边界模型。

### 第三阶段

- 新增 `readSheetObjects`；
- 完成 Excel Table 和透视表对象模型后，再接入对应摘要；
- 复用现有 ChartSpec 和 Excel 对象导入导出模型；
- 为对象读取增加导入、AI 创建、导出的一致性测试。
