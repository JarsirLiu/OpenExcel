...
- Sheet mutation tools produce two projections after a successful execution: the model-visible result may be compacted by its tool result budget, while the opaque `eventData` projection keeps the complete Sheet delta and revision for Web synchronization. Web applies `eventData`; it must not reconstruct a mutation from the compact model result.
